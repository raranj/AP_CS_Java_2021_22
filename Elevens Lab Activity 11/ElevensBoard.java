...
public class ElevensBoard extends Board {
	...
	public boolean isLegal(List<Integer> selectedCards) {
		...
		if (selectedCards.size() == 2) {
			return !((findPairSum11(selectedCards)).isEmpty()); // if a pair has been found
		} else if (selectedCards.size() == 3) {
			return (findJQK(selectedCards).size() == 3); // if there is a J, Q, and K
		} else {
			return false;
		}
	}
	...
	public boolean anotherPlayIsPossible() {
		...
		return ((!((findPairSum11(cIndexes)).isEmpty())) || (findJQK(cIndexes).size() == 3));
	}
	...
	private ArrayList<Integer> findPairSum11(List<Integer> selectedCards) {
		...
		ArrayList<Integer> cardIndices = new ArrayList<Integer>(0);
		for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
			int k1 = selectedCards.get(sk1).intValue();
			for (int sk2 = sk1 + 1; sk2 < selectedCards.size(); sk2++) {
				int k2 = selectedCards.get(sk2).intValue();
				if (cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) {
					cardIndices.add(k1); // add the indices to the empty array list and return 
					cardIndices.add(k2);
					return cardIndices;
				}
			}
	...
	private ArrayList<Integer> findJQK(List<Integer> selectedCards) {
		...
		// if there is a J, Q, and K, add them to the array list and return
		if ((jackIndex != -1) && (queenIndex != -1) && (kingIndex != -1)) {
		    cardIndices.add(jackIndex);
		    cardIndices.add(queenIndex);
		    cardIndices.add(kingIndex);
                }
		return cardIndices;
	}
	...
	private boolean playPairSum11IfPossible() {
		/* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
		List<Integer> cIndexes = cardIndexes();
		List<Integer> foundSumIndexes = findPairSum11(cIndexes);
		
		if (foundSumIndexes.isEmpty() == false) {
		    replaceSelectedCards(foundSumIndexes);
		    return true;
		}
	...
	private boolean playJQKIfPossible() {
		/* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
		List<Integer> cIndexes = cardIndexes();
		List<Integer> foundJQKIndexes = findJQK(cIndexes);
		
		if ((foundJQKIndexes.size() == 3)) {
		    replaceSelectedCards(foundJQKIndexes);
		    return true;
		}
